...
describe('ProductCard component', () => {
    it('Renders product card in default', () => {
        const element = render(<ProductCard variant='default' product={product} rating={{ average: 4.7, count: 1231 }} />);

        expect(element.queryByRole('button')).not.toBeInTheDocument();
        expect(element.getByRole('heading', { level: 3 })).toHaveTextContent('Sample Product');
    });

    it('Renders product card as add to card variant', () => {
        const element = render(<ProductCard variant='withAddToCart' product={product} rating={{ average: 4.7, count: 1231 }} />);
    ...
    it('Renders product card without rating', () => {
        ...
        expect(queryByTestId(element.container, 'product-card-rating')).not.toBeInTheDocument();
    });
});
